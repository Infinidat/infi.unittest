...
class SomeTest(TestCase):
    @parameters.iterate('x', {})
    def test(self, x):
        pass
""".format(list(range(self.num_cases))))
    def _get_test_dir_name(self):
        for i in itertools.count():
            name = 'auto_test_dir_{}'.format(i)
            if name not in sys.modules:
                return name
    def test__load_tests_from_name_class(self):
        self._test__load_tests_from_name('{}.sometest.SomeTest'.format(self.test_dir_name))
    def test__load_tests_from_name_method(self):
        self._test__load_tests_from_name('{}.sometest.SomeTest.test'.format(self.test_dir_name))
